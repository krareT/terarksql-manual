...
主 innodb，从 terarkdb 构架下先执行操作 10 分钟，切换主从，再执行操作 3 分钟，检查一致性，然后再切换回主 innodb，从 terarkdb，如此重复 3 次，主、从数据库的数据能保持一致。

主 terarkdb，从 terarkdb 构架下执行同上操作，主、从数据库的数据也能保持一致。

## 稳定性

稳定性方面的测试，我们构建了一个测试用程序，使用多个线程随机向 MyTerark 里发送包含建表、删表、改表、插入、删除等操作。持续跑了10个小时，MyTerark 依然可以正常运行。程序如下：
